    ...
    try {
      ...
      res.status(201).json({
        ...
        data: {
          ...
          rating_category: survey.getRatingCategory ? survey.getRatingCategory() : 
            (survey.rating >= 4 ? 'Excellent' : survey.rating >= 3 ? 'Good' : 'Poor'),
          nps_category: survey.calculateNPS ? survey.calculateNPS() : 
            (survey.rating >= 4 ? 'Promoter' : survey.rating <= 2 ? 'Detractor' : 'Passive'),
  ...
  async getSurveyStats(req, res) {
    try {
      ...
      const recentSurveys = surveys.slice(0, 10).map(s => ({
        ...
        rating: s.rating,
        rating_category: s.getRatingCategory ? s.getRatingCategory() : 
          (s.rating >= 4 ? 'Excellent' : s.rating >= 3 ? 'Good' : 'Poor'),
        comments: s.comments ? 
